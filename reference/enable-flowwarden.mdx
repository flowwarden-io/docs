...
|-------|-----------|----------|
| `FlowWardenAutoConfiguration` | Always | `ChangeStreamBeanPostProcessor`, `FlowWardenConfigurationValidator`, `CheckpointStore` |
| `ImperativeFlowWardenAutoConfiguration` | `flowwarden.default-mode=IMPERATIVE` (or missing) | `ImperativeStreamManager` |
| `ReactiveFlowWardenAutoConfiguration` | `flowwarden.default-mode=REACTIVE` | `ReactiveStreamManager` |
...
3. **Handler return types** — `void` handlers in `REACTIVE` mode and `Mono<Void>` handlers in `IMPERATIVE` mode are rejected
