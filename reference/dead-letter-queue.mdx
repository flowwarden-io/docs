...
public class OrderStreamHandler {
    ...
    @OnError
    void onError(ChangeStreamContext<Order> ctx, Throwable error) {
        log.error("Order {} failed: {}", ctx.getDocumentKey(), error.getMessage());
        // Manually send to DLQ with a custom reason
        ctx.sendToDlq("Processing error: " + error.getMessage());
    }
}
```
...
public class OrderStreamHandler {
    ...
    @OnError
    Mono<Void> onError(ChangeStreamContext<Order> ctx, Throwable error) {
        log.error("Order {} failed: {}", ctx.getDocumentKey(), error.getMessage());
        ctx.sendToDlq("Processing error: " + error.getMessage());
        return Mono.empty();
    }
