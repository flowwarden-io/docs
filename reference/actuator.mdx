...
Returns the status of all registered streams:
...
|-------|------|-------------|
| `streams` | `Map<String, String>` | Each stream name mapped to `"UP"` or `"DOWN"` |
...
### POST /actuator/flowwarden/\{streamName\}/stop

Stops a specific stream (emergency action):
...
<Warning>
  Only `stop` is available via the Actuator endpoint. For `start`, `restart`, `pause`, `resume`, and other lifecycle actions, use the **FlowWarden Console**. This ensures operational traceability and audit logging.
</Warning>

Attempting any action other than `stop` returns an error:

```json
{
  "stream": "order-stream",
  "error": "Only 'stop' is available locally. For 'start', 'restart', and other actions, use the FlowWarden Console."
}
```
