...
<Warning>
  A method may only carry **one** typed handler annotation. Combining annotations like `@OnUpdate` and `@OnReplace` on the same method causes a `BeanCreationException` at startup.

  ```java
  // DON'T — this fails at startup
  @OnUpdate
  @OnReplace
  void handle(ChangeStreamContext<Order> ctx) { ... }
  ```

  Instead, use separate methods for each operation type.
</Warning>
...
| "carries both @OnUpdate and @OnReplace" | Method has multiple typed handler annotations. | Use one annotation per method — split into separate methods. |
