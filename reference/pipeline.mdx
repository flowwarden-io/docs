...
```
Oplog complet
  └─► Change Stream sur la collection ciblée    ← filtre implicite MongoDB
        └─► @Pipeline (server-side)              ← réduit le trafic réseau
              └─► FlowWarden reçoit les events matchés
                    └─► @Filter (application-side) ← logique Java complexe
                          └─► Handler reçoit les events finaux
```
