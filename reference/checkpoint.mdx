...
```
┌──────────────────────────────────────────────────────────────┐
│  Event E1 received   → lastSeenToken = E1                    │
│  Event E1 processed  → lastProcessedToken = E1               │
│  Event E2 received   → lastSeenToken = E2                    │
│  ── crash ──                                                 │
│  Restart             → resume from lastProcessedToken (E1)   │
│  Event E2 re-delivered and processed                         │
└──────────────────────────────────────────────────────────────┘
```
...
Global defaults can be set in `application.yml` and overridden per stream with the annotation:
...
<Tip>
  Per-stream YAML overrides are also supported via `flowwarden.streams.{name}.checkpoint.*`. See the [Configuration reference](/reference/configuration) for the full resolution hierarchy.
</Tip>
