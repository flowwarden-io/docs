...
flowchart TD
    ...
    B --> C["ChangeStreamBeanPostProcessor\n(BeanPostProcessor)"]
    C -->|"scans beans at startup"| D["@ChangeStream beans\n(handlers, pipeline, filter,\ncheckpoint, retry, dlq)"]
    D --> E["StreamRegistry\n(ChangeStreamDefinition[])"]
    E -->|"ApplicationReadyEvent"| F["StreamManager\n(Imperative or Reactive)"]
    F -->|"subscribe"| G[("MongoDB\nChange Stream")]
```
...
```mermaid
flowchart TD
    A[("MongoDB Oplog")] -->|"implicit collection filter"| B["@Pipeline\nserver-side aggregation\nevaluated once at stream start"]
    B -->|"only matching events sent over the wire"| C["FlowWarden receives event"]
    C --> D{"@Filter\napplication-side predicate\nper event"}
    D -->|"false → event discarded"| X(["dropped"])
    D -->|"true → continue"| E{"Route by\noperation type"}
...
<Tabs>
  ...
  <Tab title="Combined — Funnel approach">
    ...
    ```mermaid
    flowchart LR
        A[("Full oplog")] -->|"server-side"| B["@Pipeline\nMongoDB aggregation"]
        B -->|"app-side"| C["@Filter\nJava predicate"]
        C --> D["Handler"]
    ```
  </Tab>
