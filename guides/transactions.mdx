...
public class OrderProcessor {
    ...
    void handle(Order order, ChangeStreamContext<Order> ctx) {
        // 1. Business write — participates in the transaction
        mongoTemplate.save(new ProcessedOrder(order), "processed_orders");

        // 2. Checkpoint — participates in the SAME transaction
        ctx.saveCheckpointNow();

        // If this method completes normally:
        //   -> both writes are committed atomically
        // If an exception is thrown:
        //   -> both writes are rolled back, event will be retried
    }
}
```
