...
@Checkpoint(
    ...
    onHistoryLost       = OnHistoryLost.FAIL            // when resume token expires from oplog
)
```
...
| `onHistoryLost` | `OnHistoryLost` | `FAIL` | Strategy when the saved resume token has expired from the oplog. See [@Checkpoint reference](/reference/checkpoint#onhistorylost). |
